        ...
        while not self._is_canceling:
            ...
            if len(lines) == 2:
                ...
                raise ProcessingError(
                    f"Unexpected number of lines in cell ({len(lines)})"
                )
...
class MathSciNetFetcher(Fetcher):
    ...
    @staticmethod
    def _extract_info(
        info: Optional[str], alt_info: Optional[str], extra_info: Optional[str]
    ...
    def fetch(self) -> Iterator[Journal]:
        ...
        with pdfplumber.open(cached_file) as pdf:
            ...
            def normalize_field(s: str) -> str:
                ...
                for line in s.splitlines():
                    if last_line is not None:
                        if len(last_line) == 0:
                            pass
                        elif len(last_line) > 1 and unicodedata.category(
                            last_line[-1]
                        ) in ("Pc", "Pd"):
                            pass
                        else:
            ...
            for entry in get_entries():
                ...
                symbols = set(
                    self._translate_symbol(symbol)
                    for symbol in entry_match.group("symbols")
                )
                ...
                name, alt_name, publisher = self._extract_info(
                    info, alt_info, extra_info
                )
