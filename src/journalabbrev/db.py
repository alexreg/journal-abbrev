...
class Journal(DBObject, metaclass = DBObjectMeta):
	...
	def matches(self, key: str, value: Union[str, Collection[str], Pattern]) -> bool:
...
class JournalList():
	...
	def query(self, key: str, value: Union[str, Pattern]) -> Iterator[Tuple[JournalID, Journal]]:
		...
		if key == Journal.names_key:
			...
			names = None
			if isinstance(value, str):
				names = (cast(str, value),)
			elif isinstance(value, Collection):
				names = cast(Collection[str], value)

			if names is not None:
				return filter(None, (name_to_journal(name) for name in names))

		return ((key, value) for key, value in self if value.matches(key, value))

	def query_one(self, key: str, value: Union[str, Pattern]) -> Optional[Tuple[JournalID, Journal]]:
		return next(self.query(key, value), None)
	...
	def rebuild_indexes(self):
		...
		self._jdb._journal_names_index_cf = self._jdb._db.create_column_family(self._jdb._journal_names_index_cf_name, self._jdb._column_families[self._jdb._journal_names_index_cf_name])

		for id, journal in self:
			for name in journal.names:
				self._jdb._db.put((self._jdb._journal_names_index_cf, _pack(name)), _pack(id))
...
class JournalDB(EventEmitter):
	...
	def repair(self):
		# TODO: implement
		pass
		# self._db.repair_db(db_name, opts)
	...
	def open(self):
		...
		self._db = rocksdb.DB(self._filename, db_opts, column_families = self._column_families)
