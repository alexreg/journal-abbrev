...
def normalize_char(char: PDFChar, interpreter: PDFPageInterpreter, proc_pdf_char: Optional[ProcessPDFCharFn] = None) -> Optional[PDFChar]:
	text = char["text"]
...
def extract_text2(chars: Iterable[PDFChar], interpreter: PDFPageInterpreter, proc_pdf_char: Optional[ProcessPDFCharFn] = None, min_tab_width: Optional[Decimal] = None, x_tolerance: Decimal = DEFAULT_X_TOLERANCE, y_tolerance: Decimal = DEFAULT_Y_TOLERANCE) -> str:
	...
	for line_chars in lines:
		line_chars = (normalize_char(char, interpreter, proc_pdf_char) for char in line_chars)
		...
		for char in sort_line_chars(line_chars, interpreter):
			if last_char is not None and last_char["text"] is not None:
				if min_tab_width is not None and char["x0"] > last_char["x1"] + min_tab_width:
					text.write("\t")
