...
def find_fetch_sources() -> None:
    ...
    fetcher_classes = (
        typ
        for name, typ in inspect.getmembers(journalabbrev.fetcher)
        if inspect.isclass(typ) and issubclass(typ, Fetcher) and hasattr(typ, name_attr)
    )
    fetcher_map = {
        normalize_name(getattr(typ, name_attr)): typ for typ in fetcher_classes
    }
...
def get_journal_queries(args: Namespace) -> Iterator[Union[JournalID, JournalQuery]]:
    if args.journals:
        for journal in args.journals:
            ...
            pattern = re.compile(rf"^{name}$", RegexFlag.IGNORECASE)
        ...
        for journal_json in journal_list_json:
            ...
            if is_regex:
                query_value = re.compile(rf"^{value}$", RegexFlag.IGNORECASE)
            else:
                query_value = value
...
def fetch_source(name: str, jdb: JournalDB, overwrite: bool = False) -> bool:
    ...
    for journal in journals:
        ...
        journal.iso4 = sub_or_none(
            no_space_after_dot_regex,
            lambda m: (". " if str.isupper(m.group(1)) else ".") + m.group(1),
            journal.iso4,
        )
        ...
        if len(found_journals) == 0:
            ...
            try:
                ...
                attr = ", ".join(attr)
                warn(
                    f"failed merge for journal #{id} ({journal_names}): key '{attr}' has base value `{base}` but new value `{nxt}`"
                )
                num_warnings += 1
            else:
            ...
            warn(
                f"multiple journals in database with names matching {journal_names_str}; not merging"
            )
            num_warnings += 1
    ...
    info(
        f"added {num_journals_added:,} journal(s) to database; updated {num_journals_updated:,} journal(s) in database; {num_warnings:,} warning(s)"
    )
    return True
...
def cmd_info(jdb: JournalDB, args: Namespace) -> None:
    ...
    info(
        f"total # of indexed journal names: {sum(1 for _ in jdb.journals.iter_name_index()):,}"
    )
...
def main() -> None:
    ...
    parser = ArgumentParser(
        description=f"Generates the database of journal abbreviations."
    )
    ...
    parser.add_argument(
        ...
        "-u",
        action="store_true",
        help=f"force upgrading the database schema",
    )

    subparsers = parser.add_subparsers(help=f"subcommands")

    cmd_reserialize_parser = subparsers.add_parser(
        "reserialize", help=f"reserialize all entries in database"
    )
    cmd_reserialize_parser.set_defaults(subcommand=cmd_reserialize)

    cmd_delete_indexes_parser = subparsers.add_parser(
        "delete-indexes", help=f"delete database indexes"
    )
    cmd_delete_indexes_parser.set_defaults(subcommand=cmd_delete_indexes)

    cmd_rebuild_indexes_parser = subparsers.add_parser(
        "rebuild-indexes", help=f"rebuild database indexes"
    )
    ...
    cmd_add_parser = subparsers.add_parser(
        "add", help=f"adds journal(s) to the database; takes JSON as input"
    )
    cmd_add_parser.set_defaults(subcommand=cmd_add_journals)
    cmd_add_parser.add_argument(
        "--overwrite",
        "-o",
        action="store_true",
        help=f"overwrite pre-existing entry if one exists",
    )

    cmd_remove_parser = subparsers.add_parser(
        "remove",
        help=f"removes journal(s) from the database; optionally takes JSON as input",
    )
    ...
    cmd_remove_parser.add_argument(
        ...
        type=str,
        nargs="*",
        help=f"the journals to remove (IDs or regexes for full name)",
    )

    cmd_get_parser = subparsers.add_parser(
        "get",
        help=f"gets entres for journal(s) from the database; optionally takes JSON as input",
    )
    ...
    cmd_get_parser.add_argument(
        ...
        type=str,
        nargs="*",
        help=f"the journals to get (IDs or regexes for full name)",
    )

    cmd_list_fetch_sources_parser = subparsers.add_parser(
        "list-sources", help=f"lists fetch sources"
    )
    cmd_list_fetch_sources_parser.add_argument(
        "--url",
        "-u",
        dest="show_url",
        action="store_true",
        help=f"show URLs of sources",
    )
    cmd_list_fetch_sources_parser.set_defaults(subcommand=cmd_list_fetch_sources)

    cmd_fetch_parser = subparsers.add_parser(
        "fetch", help=f"fetches source(s) to add to the database"
    )
    cmd_fetch_parser.set_defaults(subcommand=cmd_fetch_sources)
    cmd_fetch_parser.add_argument(
        "sources", metavar="SOURCE", type=str, nargs="+", help=f"the sources to fetch"
    )
    cmd_fetch_parser.add_argument(
        "--overwrite", "-o", action="store_true", help=f"overwrite pre-existing entries"
    )
