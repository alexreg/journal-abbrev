...
def find_journal(jdb: JournalDB, name: str) -> Optional[Tuple[JournalID, Journal]]:
    ...
    if series := match.group("series"):
        pattern.write(rf" (series )?")
        pattern.write(series.casefold())
    if subtitle := match.group("subtitle"):
        pass

    name_regex = re.compile(rf"{pattern.getvalue()}(?:\:\s.*)?")
    return jdb.journals.query_one(Journal.names_key, name_regex)


def gen_sourcemap_map(
    output_io: IO, journal: Journal, journaltitle: str, abbrev: str, issn: str
) -> None:
    new_journaltitle = abbrev or journaltitle
    issn_step_code = f"\step[fieldset = issn, fieldvalue = {{{issn}}}]" if issn else ""

    journaltitle_pattern = (
        re.escape(journaltitle).replace(r"\ ", r"\s").replace(r"\&", r"\\\x26")
    )
...
def cmd_proc_bibs(
    ...
    args: Namespace,
    *,
    output_format_arg: Action,
    output_filenames_arg: Action,
) -> None:
    ...
    output_filenames = args.output_filenames or [
        get_output_filename(filename) for filename in filenames
    ]
    if len(output_filenames) != len(filenames):
        raise ArgumentError(
            output_filenames_arg,
            f"Expected {len(filenames)} filename(s); got {len(output_filenames)}",
        )
...
def main() -> None:
    ...
    parser = ArgumentParser(
        description=f"Abbreviates journal names in bibliography files."
    )
    parser.add_argument(
        "filenames",
        metavar="FILE",
        type=str,
        nargs="*",
        help=f"a path to a bibliography (`.bib`) file to read",
    )
    parser.add_argument(
        "--silent", "-s", action="store_true", help=f"do not write messages to stderr"
    )
    parser.add_argument(
        "--online",
        "-n",
        action="store_true",
        help=f"query online databases if no match is found in the local database",
    )
    parser.add_argument(
        "--abbrev-type",
        "-t",
        ...
        choices=["iso4", "coden"],
        default="iso4",
        help=f"the type of abbreviation to output {default_arg_help}",
    )
    output_format_arg = parser.add_argument(
        "--output-format",
        "-f",
        type=str,
        choices=["bib", "sourcemap"],
        default="bib",
        help=f"the format of the output to generate {default_arg_help}",
    )
    output_filenames_arg = parser.add_argument(
        "--output-filenames",
        "-o",
        metavar="OUTPUT_FILE",
        type=str,
        nargs="+",
        help=f"the path to the bibliography (`.bib`) file to write, one per input file",
    )
